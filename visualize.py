...
class Visualize:
    def __init__(self, world_data):
        self.frame = Tk()
        self.canvas = Canvas(
            self.frame, width=FRAME_WIDTH, height=FRAME_HEIGHT)
        self.canvas.grid()
    ...
    def draw_world(self):
        ...
        for row in range(nrows):
            for col in range(ncols):
                self.vis_cells[row][col] = self.canvas.create_rectangle(
                    FRAME_MARGIN + self.cell_w * col, FRAME_MARGIN + self.cell_h * row, FRAME_MARGIN + self.cell_w * (col+1), FRAME_MARGIN + self.cell_h * (row+1))
                if self.world.cells[row][col] == IS_ROCK:
                    self.canvas.itemconfig(
                        self.vis_cells[row][col], fill='gray', width=2)
        ...
        for crow in range(self.world.h):
            for ccol in range(self.world.w):
                ...
                if(cell != UNOCCUPIED and not self.world.is_blocked(crow, ccol)):
                    ...
                    self.aindx_obj[cell] = self.canvas.create_oval(
                        x1, y1, x2, y2, fill=COLORS[cell])
                    gy, gx = self.world.aindx_goal[cell]
                    goal_cell = self.vis_cells[gy][gx]
                    self.canvas.itemconfig(
                        goal_cell, outline=COLORS[cell], width=4)
