...
class GridWorld:
        ...
        if agents_sng:
            ...
            for (sy, sx, gy, gx) in agents_sng:
                ...
                if(not self.is_blocked(sy, sx) and not self.is_blocked(gy, gx)):
                    ...
                    raise Exception(
                        'Failure! agent index: ' + str(nagents + 1))
                    return False
            return True
        return False
    ...
    def is_blocked(self, y, x):
        # print 'Cell :', y, x
        if not self.is_validpos(y, x):
            return True
        if(self.cells[y][x] == IS_ROCK):
            return True
        return False
    ...
    def tyx_dist_heuristic(self, a, b):
        ...
        if(a[0] == ANY_TIME or b[0] == ANY_TIME):
            t_dist = yx_dist/WAIT_FACTOR
        else:
            t_dist = (abs(a[2] - b[2])) * int(yx_dist > 0)
